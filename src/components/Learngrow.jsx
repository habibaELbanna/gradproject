...
const VIDEOS = [
  {
    thumb: thumb1,
    duration: '3:45',
    desc: 'Learn how to create effective procurement posts that attract quality vendors',
    views: '1,234 views',
    url: '#',
  },
  {
    thumb: thumb2,
    duration: '3:45',
    desc: 'Discover strategies to write compelling proposals and stand out from competition',
    views: '1,567 views',
    url: '#',
  },
];
...
export default function LearnGrow() {
  ...
  return (
    <section className="learn" ref={sectionRef}>
      <div className="learn__label-wrap">
        <SectionLabel title="Learn & Grow" number="006" />
      </div>

      <h2 className={`learn__title${visible ? ' learn__title--visible' : ''}`}>
        MASTER SELA<br />IN MINUTES
      </h2>

      <p className={`learn__sub${visible ? ' learn__sub--visible' : ''}`}>
        Watch quick tutorials to get the most out of the platform
      </p>
      ...
      <div className="learn__cards">
        {VIDEOS.map((v, i) => (
          <a
            key={i}
            href={v.url}
            className={`learn__card${visible ? ' learn__card--visible' : ''}`}
      ...
      <div className={`learn__footer${visible ? ' learn__footer--visible' : ''}`}>
        <a href="https://youtube.com" className="learn__yt-link" target="_blank" rel="noreferrer">
          <svg width="22" height="16" viewBox="0 0 22 16" fill="none">
            <path d="M21.5 2.5C21.2 1.3 20.3 0.4 19.1 0.1C17.4 0 11 0 11 0C11 0 4.6 0 2.9 0.1C1.7 0.4 0.8 1.3 0.5 2.5C0 4.2 0 8 0 8C0 8 0 11.8 0.5 13.5C0.8 14.7 1.7 15.6 2.9 15.9C4.6 16 11 16 11 16C11 16 17.4 16 19.1 15.9C20.3 15.6 21.2 14.7 21.5 13.5C22 11.8 22 8 22 8C22 8 22 4.2 21.5 2.5ZM8.8 11.4V4.6L14.6 8L8.8 11.4Z" fill="#00A7E5"/>
          </svg>
          Watch More on YouTube →
        </a>
        <span className="learn__subs">1.2K subscribers</span>
      </div>
    </section>
  );
}